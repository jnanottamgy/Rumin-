...
- **Not validated against outcomes.** No parameter has been estimated from data and no
  ...
  assumptions with neutral defaults. Since Phase 9 the [verification register](verification.md)
  shows what *has* been checked — hand-calculated reference cases, stated properties,
  documented limits, reproducibility — and lists this beside it. Estimation and back-testing
  were not attempted in Phase 9: no observation is stored in this environment and the sample
  companies are fictional, so there is nothing honest to estimate from or test against.
...
| Deterministic runs | A run is one path; on the Simulation page uncertainty is shown by one-at-a-time sensitivity only | Done for the Scenario Lab in Phase 9: Monte Carlo under stated distributions, with its own recorded seed, on a stored execution ([advanced analysis](../scenario-lab/advanced-analysis.md)); `simulation_runs.random_seed` stays unused. Correlated draws and variance-based indices are next |
| One-at-a-time sensitivity | On the Simulation page, no interactions between inputs; low and high are not a confidence interval | Done for the Scenario Lab in Phase 9: two-quantity grids with the interaction term |
...
| Coefficients are inputs | The engine never infers an elasticity or a lag from the graph or from data | Estimation from stored series, with its own provenance, once observations are stored (not possible in this environment in Phase 9) |
