...
- **Grounding checks figures, not reasoning.** A model's sentence can cite the right evidence
  ...
  Forbidden phrasing is caught; a subtle misreading is not. The grounded composer's templates
  do not have this problem; a model's answers should be read with their sources, which is
  why the margin sits beside them.
...
- **One process's pool.** The worker pool and the daily token budget are per API process;
  several processes would each apply their own limits. A shared queue and budget are future
  work.
