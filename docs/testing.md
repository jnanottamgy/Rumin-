...
| `test_scenario_lab.py` | 53 | The specification's hash ignores how numbers were typed and round-trips through storage; malformed scenarios and stress cases that break the change rules **refused, not clipped**; every profile matches its model and **no item is claimed twice**; templates built only on implemented models; applicability — by default only when the graph states the exposure, never without it, only when included without a company; every change must be modelled; the evidence and stored-data constraints; cautions for models touching the same costs; the companies tied to the changes; **the lines add the models' items as worked out by hand**; one model agrees with the Lab; interest alone holds operating profit at its baseline; stress cases reuse the models and assumptions; the pathway shows only what the engine computed; timeline events; stages recorded and every model run stored; the same version twice gives the same hashes; cancellation stores nothing; the time limit stops an execution; a version that cannot run fails with its plan; a final execution never runs again, and **a final state is never overwritten** (a run whose execution another process made final stops and stores nothing; recovery leaves an execution that finished in the meantime); the runner refuses work beyond its capacity, recovers executions left by a stopped server and drops queued work when stopped; the threaded runner; one-at-a-time sensitivity across models, points skipped and limits enforced, and **a shared figure's variation reaching the margins** (the Phase 9 correction: operating margin 13.675 / 276.925 at revenue 270,000,000); comparisons difference only like with like; a changed stored result is detected |
| `test_scenario_lab_statistics.py` | 39 | Phase 9's statistics, pure: the generator gives the documented values for seed 42; a seed reproduces every draw and another does not; seeds outside 0 … 2⁵³ − 1 refused; each distribution's own mean and variance; **draws follow their distribution (Kolmogorov–Smirnov against the exact CDF, fixed seeds)** and a deliberately biased sampler fails the same check; discrete frequencies against their weights (χ²); the quantile inverts the CDF; improper distributions refused; **percentiles equal Excel's `PERCENTILE.INC`**; percentile intervals report their exact binomial coverage and **contain the true percentile as often as stated** (checked over repeated samples); ranks share ties and Spearman matches a hand calculation; the histogram covers every value; the running mean ends at the sample mean; the halves compared in standard errors; shares against a threshold |
| `test_scenario_lab_analyses.py` | 21 | Monte Carlo and grids on the reference execution: **a linear line matches the exact mean and standard deviation** (repo-rate change uniform on [0, 1]: mean 375,000 within 4 standard errors, SD 216,506.35 within 5 %, rank correlation 1); a seed reproduces the analysis exactly; draws that break a model's rule rejected and counted by rule; fewer than 100 valid draws give no summary; distributions checked before any draw (8 cases: none, an unknown quantity, a duplicate, a reversed range, beyond an input's range, too many decimals, a continuous distribution for whole months, too many quantities); the limits; whole-month inputs draw whole months; **the crude × rupee interaction matches a hand calculation (∓137,500)**; quantities acting on different lines add up (no interaction); a cell that breaks a rule is skipped; axes checked first |
| `test_scenario_analyses_api.py` | 16 | What an execution can vary, with limits; a Monte Carlo analysis stored, read, listed and reproduced; a seed chosen and recorded when none is given; a grid through the API; **invalid requests refused with the field and nothing stored** (8 cases, a seed of 2⁵³ among them); unknown executions and analyses; **a changed stored request is not reproduced**; **at most two analyses compute at once (429)**; the sensitivity method recorded and older results carrying their caveat |
| `test_simulation_verification.py` | 12 | **Every registered model version passes its checks**; every registered model has checks; deliberately broken models fail the right checks — a wrong total fails the worked example and the monthly check, an effect without a cause fails *no change, no effect*, a bridge that does not close is reported, not raised; the command reports every version; the register through the API |
...
| `lib/contract` | 56 | Fixtures conform to the contract — the simulation fixtures (model, validation reports, run, explanation, provenance, verification, sensitivity, lists, a model's verification register), the Scenario Lab fixtures (templates, scenario and library, execution and history, results, pathway, explanation, sensitivity, verification, comparison, both previews, the included models' definitions, what an execution can vary, a Monte Carlo analysis, a grid, the analyses' list and their verification) and the Financial Intelligence fixtures (methods, three overviews, entities, three dossiers, the brief, a series, two stored analyses and their list, a refused threshold); the checker catches missing fields, wrong types, bad enum values, nested unions |
| `lib/decimal` | 4 | Exact grouping of every digit, rounding for display half away from zero on the digits themselves (BigInt), plain-decimal recognition; exact addition, multiplication, comparison and rounding on decimal strings (for the forms' starting points) |
...
| `pages/scenarioLab` | 19 | Against fixtures captured from a real backend: the library — templates with names and units from the API, the ones not offered with the reason, saved scenarios with their headline; comparing two executions, differenced only like with like and not ranked; a saved scenario opened on its stored execution (headline, baseline against scenario, cash flow listed as not modelled, reading writes nothing but the unstored preview); the pathway drawn from what the engine computed, **graph context listed apart with the causation caveat**, a relationship's evidence, β and lag, Escape, not-modelled relationships, the list view; the month replay (values per month, dimmed until reached, metrics "Horizon only"); plan, months, stress (changes in their units, not ranked) and explanation tabs; **a one-at-a-time analysis of chosen quantities with their values** (checkbox named by the quantity, described by its executed value and variation; the request exactly as chosen); **two quantities together** read by change and by interaction; **a Monte Carlo analysis sent only with the distributions the user states** (the request exactly as typed, the result with its percentiles, intervals, shares, histogram, rank correlations and notes); distributions checked in the browser before anything is sent; the server's reason for refusing an analysis shown; each included model's verification summary and what is not verified; an execution's reproducibility; an edit previewed on the server and labelled, then discarded back to the saved version with nothing saved; an execution followed through the server's stages until final, polling then stopping; a failed execution explained; a missing scenario and an unreachable API; a template's missing figures shown as notes until a save is attempted |
...
Phase 9 adds `analyses.integration.test.ts` (6 tests), each request built with the Lab's own
form helpers (`analysisForm.ts`) on the executed reference scenario: what the execution can
vary, with its limits; **a Monte Carlo analysis stored, read, listed and reproduced exactly**
(accepted and rejected draws adding up, the executed value −6,700,000, the histogram adding
up, percentiles in order, the seed and the runs recorded); a seed of 2⁵³ and 99 draws refused
on their fields, and a reversed range refused by the form and by the server; **a grid whose
executed cell reproduces the execution with no interaction along its row and column**;
one-at-a-time sensitivity (method 1.1.0) moving operating margin with annual revenue; and
...
For Phase 9, a scripted Chromium walk-through of a production build ran one-at-a-time
sensitivity, a grid (by change and by interaction), a Monte Carlo analysis (five quantities,
1,000 draws, a threshold), its diagnostics and configuration, *Run again and compare*
(identical), the Plan's verification and the Simulation page's register, in both themes at
1440 px, with `axe-core` on five views per theme, a keyboard reading of the histogram (arrow
keys, the bin announced, a 2 px focus ring) and the console watched; then the same views at
390 px for sideways scrolling. The first pass found **axe violations of two rules from three
causes**, all fixed: colour contrast below 4.5:1 on not-applicable model rows (a Phase 5
opacity fade, in every Lab view and both themes) and on muted notes over the dark theme's
wash (3.9:1), and a heading-order skip in the builder; and a **166 px sideways overflow on a
phone** (a long quantity name sized the whole form column). After the
fixes: no violation, no overflow, no console message. These checks are not automated yet (see
