...
| Services | `services/` | Queries and use cases: reference data, the network projection, scenarios and their versions, the Scenario Lab (plans, previews, executions, results, pathways, explanations, sensitivity, comparisons, templates) and its analyses (what an execution can vary, grids, Monte Carlo, their verification), system status, the graph's read logic (limits, filters, explanations), the simulation API (runs, explanations, verification, sensitivity, the verification register), Financial Intelligence (overview, dossiers, briefs, thresholds, stored analyses and their freshness) and the AI Analyst (its runtime, conversations and turns). |
